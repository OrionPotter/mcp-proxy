...
这个名字更直观，也更符合它的职责：作为多个 MCP 服务的统一网关。
...
见：[examples/config/servers.json](examples/config/servers.json)
...
见：[examples/config/sse-backend.json](examples/config/sse-backend.json)
...
见：[examples/config/streamable-http-backend.json](examples/config/streamable-http-backend.json)
...
- [main.py](main.py) 是统一入口
- [src/mcp_gateway/app.py](src/mcp_gateway/app.py) 负责网关生命周期和 HTTP 服务
- [src/mcp_gateway/clients.py](src/mcp_gateway/clients.py) 负责后端连接管理
- [src/mcp_gateway/proxy.py](src/mcp_gateway/proxy.py) 负责 MCP 请求路由与聚合
